...
def calculate_delta_e(img_lab, background_lab):
    ...
    for j in range(height):
        for k in range(width):
            # 确保这里传递的是单个标量值
            color1 = LabColor(lab_l=float(img_lab[j, k, 0]), lab_a=float(img_lab[j, k, 1]),
                              lab_b=float(img_lab[j, k, 2]))
            color2 = LabColor(lab_l=float(background_lab[j, k, 0]), lab_a=float(background_lab[j, k, 1]),
                              lab_b=float(background_lab[j, k, 2]))

            # 确保_delta_e_cmc函数处理的是单个颜色对
            delta_e_image[j, k] = _delta_e_cmc(color1, color2)
    return delta_e_image


def binary_threshold_delta_e(delta_e_image, threshold=20):
